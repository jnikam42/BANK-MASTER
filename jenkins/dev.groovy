		...
		stage('Deploy to Artifactory') {
			steps {
				script {
					...
					def targetDir = 'com/sarvatra/jenkins/dev/bank-product-feature-data'
			...
			steps {
				sshagent(credentials: ['dev']) {
					script {
						...
						echo "Deploying ${artifact.name} to ${DEV_TARGET_HOST}:${DEV_MICRO_V2_PATH}/applications"
						...
						sh """
                    ssh ${DEV_TARGET_USER}@${DEV_TARGET_HOST} '
                        ${DEV_MICRO_V2_PATH}/start_stop/stop.sh --s ${programName}
                    '
                """
						...
						sh """
                    scp target/${artifact.name} ${DEV_TARGET_USER}@${DEV_TARGET_HOST}:${DEV_MICRO_V2_PATH}/applications
                    ssh ${DEV_TARGET_USER}@${DEV_TARGET_HOST} '
                        ${DEV_MICRO_V2_PATH}/start_stop/start.sh --s ${artifact.name}
                    '
                """
					}
				}
